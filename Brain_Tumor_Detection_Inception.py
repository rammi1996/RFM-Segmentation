...
def plot_sample_images(X,y,image_size=(299, 299),n=50):
    for label in [0,1]:
...
inception_v3.add(inceptionV3_base)

# Add additional layers on top of InceptionV3
...
# Freeze the weights of the InceptionV3 base
...
history = inception_v3.history.history
...
inception_v3.metrics_names


# In[34]:


loss, acc = inception_v3.evaluate(x=X_test, y=y_test)
...
y_pred = inception_v3.predict(X_test)
...
y_test_prob= inception_v3.predict(X_test)
...
y_pred_binary = np.where(y_test_prob > 0.5, 1, 0)
